...
internal class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        file = file + ".txt";
        string file2 = file + "_wishlist.txt";
        try
        {
            using (StreamReader sw = new StreamReader(file, true))
            {
            }
            using (StreamReader sw = new StreamReader(file2, true))
            {
            }
        }
        catch
        ...
        descrizione_libro[] libri2 = new descrizione_libro[1000 + contatore2];
        int N = caricaLibri(libri, file);
        //int N2 = caricautenti(file2, libri2);
    ...
    static void AggiungiLibro(descrizione_libro[] biblioteca, string nomeFile, ref int N)
    {
        ...
        //controllo se valida la stringa
    ...
    static void ModificaLibro(descrizione_libro[] biblioteca, int N)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            if (biblioteca[i].titolo == titolo)
            {
                ...
                biblioteca[i].titolo = Console.ReadLine();
                Console.WriteLine("Inserisci il nuovo autore:");
                biblioteca[i].autore = Console.ReadLine();
                Console.WriteLine("Inserisci il nuovo anno:");
                biblioteca[i].anno = int.Parse(Console.ReadLine());
                Console.WriteLine("Inserisci il nuovo genere:");
                biblioteca[i].genere = Console.ReadLine();
                Console.WriteLine("Inserisci il nuovo prezzo:");
                biblioteca[i].prezzo = float.Parse(Console.ReadLine());
                Console.WriteLine("Inserisci il nuovo voto:");
                biblioteca[i].voto = int.Parse(Console.ReadLine());
            }
        }
    }
    ...
    static void SalvaUtenti(string nome, Utente[] utenti, int N)
    {
        using StreamWriter sw = new StreamWriter(nome);
        for (int i = 0; i < N; i++)
        {
            sw.WriteLine($"{utenti[i].nome_utente};{utenti[i].password};{utenti[i].lost_password_1};{utenti[i].lost_password_2};{utenti[i].lost_password_3};{utenti[i].tentativi_password}");
        }
    }
